...
from libs.model.swinUnetr import SwinUNETR
...
def main(rank, world_size, args):
    ...
    if args.model == 'ROG':
        ...
        model = UNETR(img_size=model_params['img_size'],
        ...
        model = SwinUNETR(img_size=model_params['img_size'],
                      in_channels=model_params['in_channels'],
                      out_channels=model_params['out_channels'],
                      feature_size=model_params['feature_size'],
                      use_checkpoint=model_params['use_checkpoint'],
                      ).to(rank)
    else:
    ...
    if args.adv:
        adv_loader = dataloader.Medical_data(
            False, info['data_file'], info['root'], info['val_size'],
    ...
    if training:
        ...
        for epoch in range(epoch + 1, args.epochs + 1):
            ...
            dice_v= dice[0][0]
            dice_b = dice[1][0]
            mean = dice[-1][0] #sum(dice) / len(dice)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_ver', type=str, default='/home/pitsiorl/Shiny_Icarus/data/dataset/',
                        help='Path to data')
