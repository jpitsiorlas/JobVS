...
def val(args, model, loader, criterion, metrics, rank):
    ...
    return epoch_loss.avg, [dice_v, dice_b,mean_dice]


def test(info, model, loader, images_path, test_file, rank, world_size,vol=None):
    '''
    ...
    if vol:
        patients = patients[patients.label.str.contains(vol)]
    ...
    for idx in range(rank, len(patients), world_size):
        ...
        # brain_prediction_logits = torch.nan_to_num(brain_prediction[1]).cpu()
        ...
        if pad is not None:
            ...
            # brain_prediction_logits = padding(brain_prediction_logits, pad, shape)
