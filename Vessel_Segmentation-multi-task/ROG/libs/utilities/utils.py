...
def save_epoch(state, mean, save_path, out_file, checkpoint, is_best):
    out_file.write('{},{},{},{},{},{},{}\n'.format(
        ...
        ''.join(str(state['dice']).split(',')), mean, state['lr']))
    out_file.flush()
...
def draw_curve(scores, metrics, res, save_path):
    plt.figure(figsize=(10, 5))
    plt.xlim([0,1])
    plt.ylim([0,1])
    for i, re in enumerate(res):
        plt.plot(list(metrics[i]) + [0], list(metrics[i+2]) + [1], label=f'{re}: {round(np.mean(scores[i])*100, 3)} % mAP')
    plt.legend()
    plt.xlabel('Recall')
